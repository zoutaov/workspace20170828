	...
	def findMethod(self,path=None):
		...
		for line in lines:
			...
			if self.isover:
				...
				self.mehtod=[] #清空上一个的
		self.__storeMethodLength() #记录每一个函数的长度
		self.__sortMethodsLen() #排序方法长度
		self.__pickBlackMethod()#选出黑名单
	def __storeMethodLength(self):
...
#****************************************************
	...
	def saveToTmpFile(self,savetime):
		blackFilePath = sys.path[0] + os.sep + 'analysis'+os.sep+'tmp'+savetime+'.txt'
		for method in self.black_list:
			print  method
			self.fileTools.saveFile(blackFilePath, method)  # 临时文件，用于查看读取的方法是否完整
	# **********************************************
...
def readWhiteFile(outDir):
	print 'callPickBlackMethod'
	# if not outDir or   not os.path.isdir(outDir):
	# 	print os.linesep +'Oops! " %s " is not dir ' %outDir +os.linesep
	# 	sys.exit(1)
	print 'root:',sys.path[0]
	whiteFilePath = sys.path[0]+os.sep+'white'+os.sep+"white.txt"
	print '*****'*10
	print 'whitepath:',whiteFilePath
	print '*****'*10
	allWhiteNames = fileUtils.FileHelper().openFile(whiteFilePath) #read white file 
	print 'all white names '
	print allWhiteNames
	print '*****'*30
	# print ''
	# tmpPathList=[]
	index =1
	while index < len(allWhiteNames) -1:
		print allWhiteNames[index]
		index +=1
		if outDir:
			smaliPathList.append(outDir+os.sep+allWhiteNames[index])
	# smaliPathList = tmpPathList
	# print '****'*30
	# for item in tmpPathList:
	# 	print item


def recordTimeStamp(timestamp):
	try:
		with open(sys.path[0]+os.sep+'timelog.txt',"a+") as fd:
			fd.write('----'*10)
			fd.write('tmp.txt record time :\n')
			fd.write('tmp'+timestamp)
			fd.write('\n')
	except Exception as e:
		show_error('__recordTimeStamp failed')
	
# call parse find
def findBlacklistMethod():
	if  smaliPathList == None or len(smaliPathList) <=0 :
		show_error('smalitPathList is empty !')
		sys.exit(1)
	# print type(smaliPathList) ,len(smaliPathList)
	timeStamp =time.strftime('%Y%m%d%H%M%S',time.localtime())
	recordTimeStamp(time) #recodr timestamp
	for smali in smaliPathList:
		parser = Parser()
		parser.findMethodAndSaveToTmp(smali)
...
def FindMethodAndSaveToTmpFile(outdir):
	readWhiteFile(outdir)
	findBlacklistMethod()
def writeWhiteAndBlackListFile(savePath):
	try:
		whitelist =fileUtils.FileHelper().openFile(whiteFilePath).insert(0,'whitelist: \n')
		blacklist = fileUtils.FileHelper().openFile(blackFilePath).insert(0,'blacklist: \n')
		whitelist.extend(blacklist)
		fd  = open(savePath,'w')
		for method in whitelist:
			fd.write(method)
		fd.close()
	except Exception as e:
		show_error( 'writeWhiteAndBlackListFile failed ')
		sys.exit(1)
		# fileUtils.FileHelper().saveFile
	return True


if __name__ == '__main__':
	print time.strftime('%Y%m%d%H%M%S',time.localtime())
	# readWhiteFile(outDir='/test')
	# findBlacklistMethod()
